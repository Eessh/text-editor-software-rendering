...
public:
  ...
  virtual int process_token(std::string& token,
                            bool* discard,
                            bool isSeparator,
                            Token* tok) override;
  ...
  virtual void
  on_token(std::string& token, bool* discard, bool isSeparator, Token* tok);

  std::vector<SyntaxToken> get_tokens();

  virtual std::vector<std::string> get_operators();
  virtual std::vector<std::string> get_keywords();
  virtual std::vector<std::string> get_separators() override;
...
SyntaxParser::SyntaxParser(
  std::string data, SyntaxParserLanguage language = SyntaxParserLanguage::C)
{
  _data = data;
  _language = language;
...
void SyntaxParser::on_token(std::string& token,
                            bool* discard,
                            bool isSeparator,
                            Token* tok)
{
...
std::vector<std::string> SyntaxParser::get_keywords()
{
  ...
  {
    return {"auto",     "break",  "case",    "char",   "const",    "continue",
  ...
  {
    return {"and",     "as",    "assert", "break",  "class",  "continue",
  ...
  {
    return {"abstract", "as",       "base",       "bool",      "break",
  ...
  {
    return {"as",     "break",  "const",  "continue", "crate",  "else",  "enum",
            "extern", "false",  "fn",     "for",      "if",     "impl",  "in",
            "let",    "loop",   "match",  "mod",      "move",   "mut",   "pub",
            "ref",    "return", "self",   "static",   "struct", "super", "true",
  ...
  {
    return {"and",      "break",   "by",     "catch", "class",  "continue",
            "debugger", "delete",  "do",     "each",  "else",   "extends",
            "false",    "finally", "for",    "if",    "in",     "is",
            "isnt",     "loop",    "no",     "of",    "off",    "on",
            "or",       "own",     "return", "super", "switch", "then",
            "this",     "throw",   "true",   "try",   "unless", "until",
            "when",     "while",   "yes"};
  }
  else if(language == SyntaxParserLanguage::Lua)
  {
    return {"and", "break",    "do",     "else", "elseif", "end",   "false",
            "for", "function", "if",     "in",   "local",  "nil",   "not",
            "or",  "repeat",   "return", "then", "true",   "until", "while"};
  }
  else if(language == SyntaxParserLanguage::ActionScript)
  {
    return {"break",     "case",       "catch",     "class",  "const",
            "continue",  "default",    "delete",    "do",     "else",
            "extends",   "false",      "finally",   "for",    "function",
            "if",        "implements", "import",    "in",     "instanceof",
            "interface", "is",         "native",    "new",    "null",
  ...
  {
    return {"as",    "break", "class",  "continue", "do",     "else",
            "false", "for",   "fun",    "if",       "in",     "interface",
            "is",    "null",  "object", "package",  "return", "super",
            "this",  "throw", "true",   "try",      "typeof", "val",
            "var",   "when",  "while"};
  }
  ...
  else if(language == SyntaxParserLanguage::Swift)
  {
    return {
      ...
      "defer",  "do",    "else", "fallthrough", "false", "for",      "func",
      "guard",  "if",    "in",   "init",        "is",    "nil",      "new",
      "return", "self",  "Self", "Self.",       "super", "switch",   "throw",
      "throws", "true",  "try",  "typealias",   "var",   "where",    "while"};
  }
  else if(language == SyntaxParserLanguage::ObjectiveC)
  {
    return {"@autoreleasepool", "@catch",          "@class",
            "@dynamic",         "@encode",         "@end",
            "@finally",         "@implementation", "@import",
            "@interface",       "@optional",       "@private",
            "@property",        "@protocol",       "@public",
...
{
  if(_currentToken.type != SyntaxTokenType::None &&
     _currentToken.value.size() > 0)
  {
    if(accept_token())
...
int SyntaxParser::process_token(std::string& token,
                                bool* discard,
                                bool isSeparator,
                                Token* tok)
{
  if(isSeparator)
  {
    ...
    {
      if(_currentToken.type == SyntaxTokenType::Comment ||
         _currentToken.type == SyntaxTokenType::MultilineComment)
      {
        _currentToken.value.append(token);
      }
    ...
    else if(token == "/" && (!isInString && !isInChar))
    {
      if(_currentToken.type == SyntaxTokenType::MultilineComment &&
         _currentToken.value.back() == '*')
      {
        _currentToken.value.append(token);
        push_token();
      }
      else if(_currentToken.type == SyntaxTokenType::Comment)
      {
        _currentToken.value.append(token);
      }
      else if(_currentToken.type == SyntaxTokenType::Separator &&
              _currentToken.value == "/")
      {
        _currentToken.type = SyntaxTokenType::Comment;
        _currentToken.value = "//";
      }
      else
      {
        _currentToken.type = SyntaxTokenType::Separator;
        _currentToken.value = token;
      }
    }
    else if(token == "\n" && (!isInString && !isInChar))
    ...
    {
      if(_currentToken.type == SyntaxTokenType::Separator &&
         _currentToken.value == "/")
      {
        _currentToken.type = SyntaxTokenType::MultilineComment;
        _currentToken.value = "/*";
      }
      else if(_currentToken.type == SyntaxTokenType::MultilineComment)
      {
        _currentToken.value.append(token);
      }
    }
    else if(std::find(_operators.begin(), _operators.end(), token) !=
              _operators.end() &&
            (!isInString && !isInChar))
    {
      if(_currentToken.type == SyntaxTokenType::Operator)
      {
        ...
        if(std::find(_operators.begin(), _operators.end(), cVal) !=
           _operators.end())
        {
  ...
  else
  {
    ...
    else
    {
      ...
      else if(std::find(_keywords.begin(), _keywords.end(), token) !=
              _keywords.end())
        _currentToken.type = SyntaxTokenType::Keyword;
      else if(std::find(_preprocessor_directives.begin(),
                        _preprocessor_directives.end(),
                        token) != _preprocessor_directives.end())
      {
        _currentToken.type = SyntaxTokenType::PreprocessorDirective;
      }
      else
