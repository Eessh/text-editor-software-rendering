...
namespace lexpp
{
    ...
    std::string to_string(SyntaxTokenType type)
    {
        switch(type)
        {
            ...
            case PreprocessorDirective: return "PreprocessorDirective";
    ...
    SyntaxParser::SyntaxParser(std::string data, SyntaxParserLanguage language = SyntaxParserLanguage::C)
    {
        ...
        _preprocessor_directives = {"#if", "#ifdef", "#ifndef", "#else", "#elif", "elifdef", "elifndef", "#endif", "#define", "#undef", "#include", "#error", "#warning", "#pragma", "#line"};
    ...
    std::vector<std::string> SyntaxParser::get_keywords()
    {
        ...
        {
            return {"alignas", "alignof", "and", "and_eq", "asm", "atomic_cancel", "atomic_commit", "atomic_noexcept", "auto", "bitand", "bitor", "bool", "break", "case", "catch", "char", "char8_t", "char16_t", "char32_t", "class", "compl", "concept", "const", "consteval", "constexpr", "constinit", "const_cast", "continue", "co_await", "co_return", "co_yeild", "decltype", "default", "delete", "do", "double", "dynalic_cast", "else", "enum", "explicit", "export", "extern", "false", "float", "for", "friend", "goto", "if", "inline", "int", "long", "mutable", "namespace", "new", "noexcept", "not", "not_eq", "nullptr", "operator", "or", "or_eq", "private", "protected", "public", "reflexpr", "register", "reinterpret_cast", "requires", "return", "short", "signed", "sizeof", "static", "static_assert", "static_cast", "struct", "switch", "synchronized", "template", "this", "thread_local", "throw", "true", "try", "typedef", "typeid", "typename", "union", "unsigned", "using", "virtual", "void", "volatile", "wchar_t", "while", "xor", "xor_eq"};
        }
    ...
    int SyntaxParser::process_token(std::string& token, bool* discard, bool isSeparator, Token* tok)
    {
        if(isSeparator)
        {
            ...
            {
                if (_currentToken.type == SyntaxTokenType::Comment || _currentToken.type == SyntaxTokenType::MultilineComment) {
                ...
                if (_currentToken.type == SyntaxTokenType::MultilineComment && _currentToken.value.back() == '*') {
                    _currentToken.value.append(token);
                    push_token();
                }
                ...
                else if (_currentToken.type == SyntaxTokenType::Separator && _currentToken.value == "/") {
                ...
                if (_currentToken.type == SyntaxTokenType::Separator && _currentToken.value == "/") {
        ...
        else
        {
            if(isInChar)
                ...
                _currentToken.type = SyntaxTokenType::String;
            else if (_currentToken.type == SyntaxTokenType::Comment || _currentToken.type == SyntaxTokenType::MultilineComment) {
                // do nothing
            }
            else 
            {
                ...
                else if (std::find(_preprocessor_directives.begin(), _preprocessor_directives.end(), token) != _preprocessor_directives.end()) {
